...
> **빌드 한 번에 세 가지 원고가 나옵니다.** `.\JKASE_main_build.cmd`를 인자 없이 실행하면 **투고본(submission)·수정본(review)·최종본(final)** PDF가 한 번에 모두 생성됩니다. 특정 모드만 필요하면 인자(`submission`/`review`/`final`)를 붙입니다. 자세한 내용은 아래 Build 절을 참고하세요.

## 투고·심사 규정 (Documents/)

한국과학교육학회지 공식 규정 사본이 들어 있습니다(참고용 — 투고 전 반드시 학회 홈페이지의
최신본을 확인하세요).

- 투고규정: [Documents/과학교육학회지_투고규정.md](Documents/과학교육학회지_투고규정.md)
- 심사규정: [Documents/과학교육학회지_심사규정.md](Documents/과학교육학회지_심사규정.md)
...
- 출력 모드는 빌드 스크립트 인자(`submission`, `review`, `final`)로 지정하거나, `JKASE_main.tex` 상단의 `\def\PAPERMODE{...}` 값으로 지정할 수 있습니다. 빌드 스크립트 인자가 파일 안 기본값보다 우선합니다.
- `submission`/`review` 모드에서는 1단 편집으로 저자 정보가 비워지고, `final` 모드에서만 2단 편집으로 저자 정보가 표시됩니다.
...
- `JKASE_main.tex`: 제목, 저자, 초록, 주제어, 문서 모드 설정. 문서 클래스는 `\documentclass[backend=biber,style=apa,sortcites=true]{jkase}`를 사용합니다.
- `jkase.cls`: 한국과학교육학회지 양식(제목/저자/초록 앞표지, 2단 조판, APA 참고문헌)을 정의하는 문서 클래스
...
설치 방법은 [3-VSCODE_TeXLive_SETUP.md](../manual/3-VSCODE_TeXLive_SETUP.md)를 참고합니다. Windows에서는 `.\JKASE_main_build.cmd`를, macOS/Linux에서는 `./JKASE_main_build.sh`를 사용합니다.
...
인자 없이 실행하면 투고본, 수정본, 최종본 PDF가 각각 생성됩니다.

- `JKASE_main_submission.pdf`: 투고본
- `JKASE_main_review.pdf`: 수정본
- `JKASE_main_final.pdf`: 최종본

특정 모드만 빌드하려면 인자를 지정합니다. 빌드가 끝나면 해당 모드의 PDF가 `JKASE_main.pdf`로도 복사됩니다.

```powershell
.\JKASE_main_build.cmd submission
.\JKASE_main_build.cmd review
.\JKASE_main_build.cmd final
```
...
Python 3.10 이상에서 아래처럼 설치합니다. 이 저장소의 [4-PYTHON_CONDA_VSCODE_SETUP.md](../manual/4-PYTHON_CONDA_VSCODE_SETUP.md)를 따라 만든 `knue-python` conda 환경에서 실행하는 것을 권장합니다.
