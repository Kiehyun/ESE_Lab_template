...
GitHub에서 템플릿 저장소를 내려받으면 여러 양식이 들어 있는 저장소 전체가 함께 받아질 수 있습니다. 실제 한국교원대학교 학위논문을 작성할 때는 `KNUE_thesis/` 하위 폴더만 자신의 논문 작업 폴더로 복사하거나, 다른 양식 폴더를 삭제하고 이 폴더만 남겨 사용하는 것이 좋습니다. 추후 학회지나 다른 기관 양식이 추가되더라도 같은 방식으로 필요한 하위 폴더 하나만 선택해 사용하면 됩니다.
...
LaTeX 설치, VS Code 설정, 빌드 실행 방법은 상위 폴더의 [README.md](../README.md)와 거기서 안내하는 설치 문서를 확인합니다. Overleaf로 작성하려면 [1-OVERLEAF_THESIS_PROJECT.md](../1-OVERLEAF_THESIS_PROJECT.md)를, 개인 PC에 직접 설치해 작성하려면 [2-VSCODE_LOCAL_THESIS_SETUP.md](../2-VSCODE_LOCAL_THESIS_SETUP.md)를 참고합니다. Windows에서 TeX Live를 설치할 때 참고할 공식 문서와 연구실 ISO 이미지 설치 방법은 `2-VSCODE_LOCAL_THESIS_SETUP.md`에 정리되어 있습니다. 이 문서는 한국교원대학교 학위논문 템플릿의 파일 구성과 작성 방법만 설명합니다.
...
부록에 코드 파일을 넣을 때는 코드 파일을 `code/` 폴더에 저장하고, 부록용 `.tex` 파일에서 해당 파일명을 참조합니다.

## 참고문헌 PDF 도구 (code 폴더)

`code/` 폴더에는 참고문헌 PDF를 관리하는 Python 스크립트 두 개가 함께 들어 있습니다.
...
Install them on Python 3.10 or newer. Running them inside the `knue-python` conda environment from this repository's [3-PYTHON_CONDA_VSCODE_SETUP.md](../3-PYTHON_CONDA_VSCODE_SETUP.md) is recommended.
