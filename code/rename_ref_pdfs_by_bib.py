...
# 이 스크립트는 저장소 루트의 공용 `code/` 폴더에 있습니다. 대상 원고 폴더
# (sub/references.bib 가 있는 템플릿/작업 폴더)로 이동(cd)한 뒤 실행하세요. 경로는
# 실행 위치(현재 작업 폴더)를 기준으로 해석합니다. 다른 폴더를 대상으로 하려면
# --bib / --ref-dir / --source-dir 등으로 직접 지정합니다.
#   예)  cd JKASE_template && python ../code/rename_ref_pdfs_by_bib.py --dry-run
ROOT = Path.cwd()
...
# 인용 키를 찾는 LaTeX aux 파일(원고 폴더의 *_main*.aux 를 자동 탐색: JKASE_main.aux,
# KNUE_thesis_main.aux, RISE_main.aux 등). 특정 파일은 --aux 로 지정.
DEFAULT_AUX = ROOT / "*_main*.aux"
