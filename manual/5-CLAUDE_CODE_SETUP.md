...
VS Code에는 Claude Code 외에도 비슷한 방식으로 동작하는 AI 채팅 확장이 있습니다. GitHub Copilot이 제공하는 **Chat**과 ChatGPT의 **Codex**도 오른쪽 패널에 탭으로 뜹니다. 오른쪽 패널에서 원하는 탭(Chat/Codex/Claude Code)을 클릭하고, 채팅창에 표 열 너비 조정·그림 삽입·문장 다듬기·빌드 오류 확인 같은 요청을 자연어로 입력하면 됩니다. 어떤 확장을 쓰든 사용법은 비슷하므로, 연구실에서 이미 쓰고 있는 도구가 있다면 그대로 사용해도 됩니다.
